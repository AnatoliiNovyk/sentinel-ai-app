...
- [x] Додано регулярний E2E smoke-check для `agent_health_probe` через `ai-gateway`: новий ops-скрипт + scheduled/manual workflow + контрактний тест, щоб рано виявляти регресії auth/reachability (`scripts/agent-health-probe-smoke.ps1`, `.github/workflows/agent-health-probe-smoke.yml`, `scripts/test-ops-scripts.cjs`).
- [x] Додано auto-merge для maintenance PR з оновленням Browserslist DB та failure-alerting для `agent_health_probe` smoke workflow через operational webhook (`.github/workflows/browserslist-db-maintenance.yml`, `.github/workflows/agent-health-probe-smoke.yml`).
- [x] Додано operational visibility для `agent_health_probe` smoke у Dashboard: workflow зберігає результат у `audit_logs`, а UI показує останній статус/HTTP/request-id/last-run; додано UI-тести (`.github/workflows/agent-health-probe-smoke.yml`, `src/pages/Dashboard.tsx`, `src/pages/__tests__/Dashboard.test.tsx`).
- [x] Розширено incident runbook окремим playbook для `agent_health_probe` smoke failures (401/timeout/unreachable), джерелами truth через Dashboard/audit_logs і manual recovery кроками (`RUNBOOK_SCAN_PIPELINE_INCIDENTS.md`).
- [x] Посилено Dashboard test hardening для probe smoke visibility: тести тепер покривають authenticated сценарій з даними `audit_logs` (status/reachable/http/request-id/url), а не лише fallback `Unknown`; також уточнено query flow у Dashboard для стабільної фільтрації до `order/limit` (`src/pages/Dashboard.tsx`, `src/pages/__tests__/Dashboard.test.tsx`).
- [x] Додано fail-safe persistence для probe smoke статусу в `audit_logs`: inline workflow-логіку винесено в окремий Node скрипт і підключено з `continue-on-error`, додано контрактні тести сценаріїв `success / audit insert fail / missing project context` (`scripts/persist-agent-probe-audit.cjs`, `.github/workflows/agent-health-probe-smoke.yml`, `scripts/test-ops-scripts.cjs`).
