import { gatewayError, isPayloadTooLarge, parseGatewayRequest, } from './contract.ts';
...
const EVENT_RATE_WINDOWS_MINUTES = [5, 15];
...
function jsonResponse(payload, requestId, status = 200, extraHeaders = {}) {
...
function logWithRequestId(requestId, message, err) {
    if (err === undefined) {
...
function incrementTelemetry(metric, requestId, statusCode) {
...
export function getAiGatewayTelemetrySnapshot() {
    return { ...telemetryMetrics };
}
export function getAiGatewayRecentEventsSnapshot(limit = RECENT_EVENTS_RESPONSE_LIMIT) {
...
function createEmptyByTypeCounter() {
    return {
        unauthorized: 0,
        invalid_json: 0,
        payload_too_large: 0,
...
export function getAiGatewayEventRatesSnapshot(nowMs = Date.now()) {
    const result = {};
...
function getAiGatewayAlertsSnapshot(eventRates) {
    const window5m = eventRates.window_5m;
    const window15m = eventRates.window_15m;
    const fallbackRatio15m = window15m.total > 0 ? window15m.by_type.provider_fallback / window15m.total : 0;
    return {
        ...
        degraded_mode: window15m.total >= ALERT_DEGRADED_MIN_TOTAL_15M &&
            fallbackRatio15m >= ALERT_DEGRADED_FALLBACK_RATIO_15M,
    };
}
function getAiGatewayOverallRiskLevel(alerts, eventRates) {
...
function getAiGatewayRecommendedActions(alerts, overallRiskLevel) {
    const actions = [];
...
export function resetAiGatewayTelemetryForTests() {
    Object.keys(telemetryMetrics).forEach((metric) => {
