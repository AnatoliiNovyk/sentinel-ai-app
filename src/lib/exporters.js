...
export function toSarif(project, scan, vulns) {
    ...
    const results = vulns.map((v) => {
        ...
        return {
            ...
            suppressions: v.status === 'accepted' || v.status === 'false_positive'
                ? [
                    {
                        kind: v.status === 'false_positive' ? 'external' : 'inSource',
                        status: 'accepted',
                        justification: v.note || (v.status === 'false_positive' ? 'Marked as false positive' : 'Accepted risk'),
                    },
                ]
                : undefined,
        };
...
export function fromSarif(raw) {
    ...
    for (const run of doc.runs) {
        ...
        if (driver?.name)
            scanner = String(driver.name).toLowerCase().replace(/\s+/g, '-');
        const rules = new Map();
        ...
        for (const res of run?.results ?? []) {
            ...
            const asset = loc?.artifactLocation?.uri ??
...
export function toCsvExport(vulns) {
    const header = [
        'id',
        'severity',
        'status',
...
export function toJsonExport(project, scan, vulns) {
    return JSON.stringify({
        ...
        triage_summary: vulns.reduce((acc, v) => {
