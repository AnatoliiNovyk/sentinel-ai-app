...
const SEV_PENALTY = { critical: 25, high: 10, medium: 4, low: 1, info: 0 } as const;
const MAX_SCORE = 100;
...
export function computeCompliance(vulns: Vulnerability[]): ComplianceResult {
  ...
  // ── MITRE ATT&CK ──────────────────────────────────────────────────────────
  const mitreRows: MitreRow[] = MITRE_TACTICS.map(t => {
    const matched = open.filter(v =>
  ...
  // ── NIST CSF ──────────────────────────────────────────────────────────────
  const nistRows: NistRow[] = NIST_FUNCTIONS.map(f => {
    const matched = open.filter(v =>
      f.keywords.some(kw =>
        norm(v.title).includes(kw) ||
  ...
  const soc2Rows: Soc2Row[] = SOC2_CRITERIA.map(c => ({
    id: c.id,
    label: c.label,
    weight: c.weight,
  ...
  const soc2Overall = Math.round(
    soc2Rows.reduce((acc, r) => acc + r.score * r.weight, 0) /
