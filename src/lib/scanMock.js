...
export async function runMockScan(userId, projectId, scanner) {
    ...
    await supabase
        ...
        .eq('id', scan.id);
    const topSeverity = summary.critical > 0 ? 'critical' : (summary.high > 0 ? 'warning' : 'success');
    const parts = [];
    if (summary.critical)
...
export const AVAILABLE_SCANNERS = [
    // Network
