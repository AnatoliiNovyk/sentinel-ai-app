...
export async function dispatchScan(
  ...
  if (scanErr || !scan) {
    console.error('[scanDispatch] Failed to create scan row:', scanErr);
    return null;
  }
  ...
  await supabase.from('scans').update({ status: 'running' }).eq('id', scan.id);
...
export async function dispatchDueSchedules(userId: string): Promise<number> {
  ...
  for (const s of due) {
    ...
    try {
      const scanId = await dispatchScan(userId, s.project_id, s.scanner, target);
      if (scanId) fired++;
    } catch (err) {
      console.error('[scheduler] Failed to dispatch scheduled scan:', err);
    }
  }
