...
import { getAiGatewayTelemetrySnapshot, handleAiGatewayRequest, resetAiGatewayTelemetryForTests, } from '../../../supabase/functions/ai-gateway/handler';
...
describe('ai-gateway admin metrics endpoint', () => {
    ...
    it('returns telemetry snapshot for GET metrics with valid admin key', async () => {
        ...
        runtime.Deno = {
            env: {
                get: (key) => {
                    if (key === 'AI_GATEWAY_ADMIN_KEY')
                        return 'admin-secret';
                    if (key === 'AI_GATEWAY_VERSION')
                        return 'test-v1';
                    return undefined;
                },
            },
    ...
    it('sets alerts to true when thresholds are exceeded', async () => {
        ...
        runtime.Deno = {
            env: {
                get: (key) => (key === 'AI_GATEWAY_ADMIN_KEY' ? 'admin-secret' : undefined),
            },
        };
        for (let i = 0; i < 5; i++) {
        ...
        const actionIds = body.recommended_actions.map((item) => item.id);
