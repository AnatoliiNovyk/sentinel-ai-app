import { Project, Scan, Vulnerability } from './supabase';

export type ReportKind = 'executive' | 'technical';
...
`;
  ...
  const ordered = [...vulns].sort((a, b) => {
    const rank = (s: Vulnerability['status']) =>
      s === 'open' ? 0 : s === 'in_progress' ? 1 : s === 'accepted' ? 2 : s === 'resolved' ? 3 : 4;
