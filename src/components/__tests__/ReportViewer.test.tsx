...
describe('ReportViewer', () => {
  ...
  describe('share button label', () => {
    it('shows "Share" text on share button when report has no share token', () => {
      render(<ReportViewer report={makeReport({ share_token: null })} onClose={vi.fn()} />);
      const shareBtn = screen.getByTitle('Generate public link');
  ...
  describe('footer stats', () => {
    it('renders "Generated by Sentinel AI Reporting Engine" in footer', () => {
      render(<ReportViewer report={makeReport()} onClose={vi.fn()} />);
      expect(screen.getByText('Generated by Sentinel AI Reporting Engine')).toBeInTheDocument();
    });

    it('shows ~2 min read for content with more than 200 words', () => {
      const longContent = Array.from({ length: 210 }, (_, i) => `word${i}`).join(' ');
      render(<ReportViewer report={makeReport({ content: longContent })} onClose={vi.fn()} />);
