...
$cleanupAttempted = $ApplyCleanup.IsPresent -and $beforeStaleJobs.Count -gt 0
$cleanupOk = $null
if ($cleanupAttempted) {
  $cleanupOk = $cleanupRes.StatusCode -ge 200 -and $cleanupRes.StatusCode -lt 300
}

$recoveryOutcome = 'no_stale_detected'
if ($beforeStaleJobs.Count -gt 0 -and -not $ApplyCleanup.IsPresent) {
  $recoveryOutcome = 'dry_run_detected_stale'
} elseif ($cleanupAttempted -and -not $cleanupOk) {
  $recoveryOutcome = 'cleanup_failed'
} elseif ($cleanupAttempted -and $cleanupOk -and $afterStaleJobs.Count -lt $beforeStaleJobs.Count) {
  $recoveryOutcome = 'cleanup_improved'
} elseif ($cleanupAttempted -and $cleanupOk -and $afterStaleJobs.Count -eq 0) {
...
if ($SendWebhook) {
  ...
  $severity = if ($summary.stale_running_jobs_after_count -gt 0 -and $ApplyCleanup) { 'warning' } elseif ($summary.stale_running_jobs_before_count -gt 0) { 'warning' } else { 'info' }
...
if ($cleanupAttempted) {
  ...
  if ($summary.stale_running_jobs_after_count -ge $summary.stale_running_jobs_before_count -and $summary.stale_running_jobs_before_count -gt 0) {
